...
if($('#dark-mode-toggle').length > 0) {
	...
	const enableDarkMode = () => {
		...
		document.getElementById('sun-light').style.display="flex";
		document.getElementById('moon-light').style.display="none";
	...
	const disableDarkMode = () => {
	  document.body.removeAttribute('data-theme', 'dark');
		...
		$("#dark-mode-toggle").children(".light-mode").addClass("active");
		document.getElementById('moon-light').style.display="flex";
		document.getElementById('sun-light').style.display="none";
...
async function openCammera(){
	...
	canvas.style.display="none";
		 ...
		 document.getElementById('webCamvideo').style.position="absolute";
}
...
function captureImage(){
	...
	canvas.style.display="none";
		 ...
		 document.getElementById('webcam_image').value = image_data_url;
		 canvas.style.display="none";
		 document.getElementById('image-upload-custs').innerHTML = '<img src="' + image_data_url + ' "/>';
}

async function openStockCammera(count){
	let video = document.querySelector("#webCamvideo"+ count);
	let canvas = document.querySelector("#canvas"+ count);
	canvas.style.display="none";
		 let stream = await navigator.mediaDevices.getUserMedia({ video: true, audio: false });
	     video.srcObject = stream;
		 canvas.style.display="block";
		 document.getElementById('webCamvideo'+ count).style.display="block";
}
	
function captureStockImage(count,captureInputId){
		let video = document.querySelector("#webCamvideo"+ count);
		let canvas = document.querySelector("#canvas"+ count);
		...
		canvas.style.display="none";
		 canvas.getContext('2d').drawImage(video, 0, 0, canvas.width, canvas.height);
		 let image_data_url = canvas.toDataURL('image/jpeg', 0.5);
		 document.getElementById(captureInputId).value = image_data_url;
		 canvas.style.display="none";
		 document.getElementById('image-upload-custs'+ count).innerHTML = '<img style="width:254px" src="' + image_data_url + ' "/>';
}
function closeWebcamImageDiv(div){
...
// barcode drag and drop start here

function draggingBarcode(barcodeClass,barcodeId,HTTP_HOST,settingId){
	$("."+barcodeClass).draggable({
		containment: "#"+barcodeId,
	  drag: function() {
	...
	$("#"+barcodeId).droppable({
	  drop: function(event, ui) {
		var pos = $(ui.draggable).attr('style')
		// $(this).addClass("ui-state-highlight")
		
		if(HTTP_HOST !='' && settingId !='' && HTTP_HOST !='undefined' && settingId !='undefined' && HTTP_HOST != null && settingId != null){
			changeDiv(HTTP_HOST,'product','barcode/label_setting',{setting_label_style:pos,setting_id:settingId,operation:'update',operation_for:'cordinate_save'},'unknown','','','DoNotShowDoNotLoad');
		}
  
	  }
	});
}
// barcode drag and drop end here
...
function showQR(textQr){
	$('#qrcodeOut').qrcode({
		text: textQr,
		render: 'image',
	});
}
function selectBarOrQr(type,text){
	if(type == 'barcode'){
	document.getElementById('qrcodeOut').innerHTML='';
	document.getElementById('barcodeCodeOut').style.display= 'block';
	}else if(type == 'qrCode'){
		document.getElementById('barcodeCodeOut').style.display= 'none';
		document.getElementById('qrcodeOut').innerHTML='';
		$('#qrcodeOut').qrcode({
			text: text,
			render: 'image'
